...
const Post = ({ post }) => {
	...
	// Safety check for post and its required properties
	if (!post || !post.user) {
		return <div className="p-4 border-b border-gray-700">No Posts Available</div>;
	}
	
	// Safety check for authUser
	if (!authUser) {
		return <div className="p-4 border-b border-gray-700">Loading user data...</div>;
	}
	
	const postOwner = post.user;
	// Add safety checks for all property accesses
	const isLiked = post.likes ? post.likes.includes(authUser._id) : false;
	const isMyPost = authUser._id === (postOwner?._id || null);
	const formattedDate = post.createdAt ? formatPostDate(post.createdAt) : "Unknown date";
